...
Validado com Playwright no **Chromium empacotado e no Google Chrome real**: 5 suítes (geração de datas contra oráculo independente em Python, interface, prévia, gravação com `fetchSupabase` stubado, e smoke de versão/ajuda), mais conferência visual nos temas claro e escuro. **WebKit/Safari não foi testado** — a mudança não toca em Service Worker além do bump nem em nada específico de engine, mas a lacuna existe.
...
**Validação do v77 (31/jul)** — confirmar com quem pediu os dias da semana: recarregar o portal (rodapé deve mostrar **v77**; se não, `/?limpar=1`), criar um evento com recorrência **Diária**, conferir que Seg–Sex já vêm marcados, que a prévia mostra a contagem de datas e o período, e que nada nasce em sábado/domingo. Testar também um subconjunto (seg/qua/sex e seg–qui) e a opção "Pular feriados nacionais". ⚠️ **Mudança de comportamento padrão**: quem usava Diária esperando os 7 dias passa a receber 5 — está dito na nota de versão, mas vale avisar se alguém reclamar de "sumiu o sábado".
