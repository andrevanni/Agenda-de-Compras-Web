...
**⚠️ v78 PRONTA NA `staging`, AINDA NÃO EM PRODUÇÃO** — 15 commits, tudo validado (7 suítes + E2E no Chrome real contra a Service Farma), mas **o merge `staging` → `main` não foi feito**. Enquanto não for, a compradora que reportou o problema **não consegue validar**: ela usa a produção, que continua na v77. Passos: (1) merge `staging` → `main` e push (deploy automático); (2) conferir na produção pelo Chrome que o rodapé mostra **v78** e a faixa "📌 Nota fixa deste fornecedor" aparece no modal de tratar agenda — se o rodapé travar na versão antiga, `/?limpar=1`; (3) só então enviar o PDF-guia (`~/Downloads/Agenda_Compras_Nota_Fixa_Fornecedor_Guia.pdf`) e pedir a validação dela.
