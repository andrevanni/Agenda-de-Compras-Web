...
- **Dias da semana na Diária (v77 — 31/jul/2026)**: escolher a recorrência **Diária** revela `newEventDiasSemanaWrap` com 7 checkboxes (`name="newEventDiaSemana"`, valores de `DIAS_SEMANA`), **Seg–Sex marcados por padrão**, mais "Pular feriados cadastrados" (desmarcado na v77; **marcado desde a v79**, e o rótulo antigo "nacionais" estava errado — sempre usou `state.feriados`) e uma prévia de quantas datas serão criadas (desde a v79 fora do bloco da diária, valendo para toda recorrência). `buildDiariaDates` ([script_main.js](frontend/script_main.js)) gera as datas reusando `nextCalendarDate` (o mesmo helper dos dias de compra do fornecedor) e **inclui a data base na varredura** — base em dia desmarcado faz a série começar no próximo dia marcado. Por isso `saveNewEvent` calcula `dates` **antes** do aviso de feriado e da checagem de conflito, que passaram a olhar `dates[0]`. Motivo: a diária gerava sábado/domingo, e ocorrência de fim de semana nunca tratada vira pendência eterna nos "Itens em Atraso" e derruba a taxa de conclusão de Outras Atividades. O JSON da coluna `recorrencia` passou a guardar `dias` e `pular_feriados` (JSONB, sem migration) — hoje ninguém lê de volta; é a base para uma futura edição de dias da série. **Escopo: só criação** — o modo edição segue sem o bloco, e nenhuma série existente foi alterada.
- **Dia útil nas periódicas + ajuste de séries existentes (v79 — 17/set/2026)**: `buildRecorrenciaDatesUteis(base, tipo, fim)` ([script_main.js](frontend/script_main.js)) substituiu `buildRecorrenciaDates` — **mensal = mesmo dia do mês** (`addMonthsClampIso`, clamp ao último dia; antes somava 30 dias e escorregava) e toda data de semanal/quinzenal/mensal que cai em sábado, domingo ou feriado cadastrado vai para `proximoDiaUtil`. O ajuste parte sempre da data **ideal**, então não acumula; a função **inclui** a data base (o chamador não prefixa `[data, ...]`). Na diária, "Pular feriados cadastrados" passou a vir marcado. A prévia (`newEventRecorrenciaPreview`) saiu do bloco da diária e vale para toda recorrência. **Botão "Ajustar dias desta série"** (bloco de escopo do modal de edição) abre `serieAjusteModal`: lê a série **do servidor** (`fetchSupabaseAll`, nunca `state.agenda`; o `serie_id` vem de `newEventEditId.dataset.serieId`, não do state, porque a ocorrência aberta pode ter sido apagada pelo próprio ajuste), `inferirModoAjusteSerie` decide pelo `recorrencia.tipo` ou, sem ele, pela mediana do intervalo **de todas as situações** (só pendentes esparsas enganariam a mediana); **diária remove** (dias desmarcados + feriados, inclusive vencidas; **preserva quem tem `nota`**), **periódica move** (só `>= hoje`; não move para dia em que o mesmo comprador já tem a série — conta como conflito). Gravação com `&tenant_id=eq.&status=eq.PENDENTE&select=id` + `return=representation` (conta o que o banco alterou de fato), DELETE em lotes de 100, guarda `_serieAjusteToken` e trava `_serieAjusteGravando` (bloqueia prévia, reabertura, Esc e duplo clique durante a gravação). Falha parcial relê a série e mostra só o que falta. A prévia avisa quando a série atinge mais de um comprador. ⚠️ **Decisão de produto (17/set/2026, mantida pelo André após a revisão):** não há como criar semanal/quinzenal/mensal **num sábado** — toda periódica vai para dia útil. Se algum cliente pedir, a saída combinada é um checkbox "Mover fins de semana e feriados para o próximo dia útil" marcado por padrão. Séries mensais antigas **não** têm o dia do mês reescrito. Motivo: comprador relatou séries antigas com sábado/domingo e a única saída era apagar tudo; em 17/set/2026 as 50 pendências de fim de semana dele foram removidas por SQL com autorização (backup antes). Spec/plano em `docs/superpowers/specs|plans/2026-09-17-ajuste-dias-uteis-series*`.
...
**Barra de ação fixa no `agendaDetailModal`** (mesma entrega, autorizada pelo usuário): a faixa nova custa 66px e o modal já era apertado. O `.modal-card` virou flex column com `overflow: hidden`, quem rola é o `.detail-scroll-body`, e o `.detail-sticky-footer` (com **Tratar Agenda**) fica fora da área de rolagem. Medido no Chrome real: o botão vai de **0% / 61% / 100%** visível (1280×800, 1440×960, 1920×1080) para **100% nas três**. O caso de 1280×800 **já estava quebrado antes desta feature** — a barra fixa conserta um problema pré-existente de densidade do modal. ⚠️ O `.close-button` (círculo ✕) é `position:absolute` compartilhado por todos os modais; em ≤980px os botões viram `width:100%` e há reserva de `padding-right` só nesse breakpoint para não colidir com ele.

**Achados dos reviews que eu não teria pego sozinho:** (1) `saveAgendaSupplierNote` tratava "não lançou" como "salvou" — `persistSupplierNote` faz `return` silencioso, sem gravar e sem lançar, quando o PATCH da coluna falha **e** `state.clientRecordId` está vazio; era o padrão "tela mentindo" que já custou caro aqui. (2) O primeiro oráculo escolhido para detectar isso usava `getSupplierNote()`, que trata `""` como falsy e cai no mapa legado — quebrava a **remoção** de nota com erro falso permanente; trocado por leitura direta de `state.suppliers[].notas_relacionamento`. (3) Callback tardio pintando sucesso no fornecedor errado — resolvido com guarda de geração no padrão do `_cargaGeracao`. (4) **Perda de texto na interação entre duas tasks**: com o editor da faixa aberto e um rascunho não salvo, clicar "Fixar neste fornecedor" e depois "Salvar nota fixa" fazia o rascunho sobrescrever tudo e o lembrete promovido sumia dos **dois** lugares — o botão Fixar passou a ser desabilitado enquanto o editor está aberto. (5) A Ajuda afirmava que o lembrete "some do painel quando o compromisso é concluído" — **falso**: nem `tratarAgendaAtual` nem `concluirCompromisso` limpam `nota`, e `renderPainel` inclui as REALIZADAs.
...
**Validação:** 7 suítes Playwright com estado stubado (120+ asserções) **mais um E2E no Google Chrome real contra os dados reais da Service Farma** — carga real, faixa exibindo a nota do banco, promoção real, releitura direta do Supabase, sobrevivência à recarga do portal e edição inline gravando; estado do tenant restaurado e conferido por fora do browser ao final. Temas claro e escuro conferidos por screenshot. **WebKit/Safari não foi testado.**

⚠️ **Ao escrever teste que grava em tenant real:** o roteiro E2E precisa abortar **antes de qualquer escrita** se a leitura do estado inicial falhar. Na primeira execução a carga demorou mais que o `wait_for_timeout` fixo, o estado original veio vazio, e o bloco de restauração gravou `null` por cima da nota real (reparado na hora). Use espera ativa (`wait_for_function`) e um guard explícito, nunca `sleep` fixo contra Supabase real.

**Minors conhecidos, não corrigidos:** (a) `mesclarNotaFixa` compara blocos separados por `\n{2,}` da nota atual contra o texto novo inteiro — promover 2× um texto **multi-parágrafo** duplica (1 linha não duplica). (b) Em fornecedor com resíduo no fallback legado de `clientes.observacoes`, remover a nota fixa não limpa a exibição, e promover ressuscita o texto legado para a coluna real — causa raiz em `getSupplierNote`, **código de risco compartilhado não alterado por falta de autorização**; medido no banco: **1 de 22 clientes tem esse resíduo e é o tenant de teste** ("teste agora"), zero clientes reais afetados. (c) ~20 linhas duplicadas entre `saveAgendaSupplierNote` e `fixarNotaNoFornecedor` — candidato a helper `persistSupplierNoteVerified()`.
...
Spec e plano em [docs/superpowers/specs/2026-07-31-dias-semana-recorrencia-diaria-design.md](docs/superpowers/specs/2026-07-31-dias-semana-recorrencia-diaria-design.md) e [docs/superpowers/plans/2026-07-31-dias-semana-recorrencia-diaria.md](docs/superpowers/plans/2026-07-31-dias-semana-recorrencia-diaria.md). Detalhe de comportamento na seção "Modal Novo Evento / Edição". **Escopo deliberado: só criação** — nenhuma série existente foi tocada; para trocar os dias de uma série antiga, o usuário exclui por "Toda a série" e recria.

**Dois bugs achados pelo review final, que eu não teria pego sozinho:**
- **`newEventRecorrenciaFim` nunca era limpo em `openNewEventModal`** — o valor sobrevivia entre aberturas do modal. A segunda série diária seguida herdava a data de fim da primeira e, se fosse anterior à nova data base, o guard novo bloqueava o Salvar ("a funcionalidade nova não funciona"). O mesmo resíduo já estragava semanal/quinzenal/mensal **antes desta feature** (criavam 1 linha em vez da série) — bug antigo, corrigido junto.
- **Laço de `buildDiariaDates` podia não progredir**: `nextCalendarDate` monta `new Date(base + "T12:00:00")` local e devolve `toISOString()`; em fuso ≥ UTC+13 isso retorna `base − 1` e o laço repetia a mesma data, gravando 500 linhas iguais. Guard de progresso (`if (prox <= current) break`) adicionado. No Brasil (UTC-3, sem horário de verão) é no-op — validado em 17 fusos.
- **Minor conhecido, não corrigido**: a causa raiz do `toISOString()` sobre data local continua em `nextCalendarDate` ([script_utils.js](frontend/script_utils.js)) e trunca série em silêncio em fuso ≥ UTC+13 (inclui Auckland no horário de verão). A função é compartilhada com os dias de compra do fornecedor — mexer nela custaria mais risco do que resolve para uma base 100% brasileira.

Validado com Playwright no **Chromium empacotado e no Google Chrome real**: 5 suítes (geração de datas contra oráculo independente em Python, interface, prévia, gravação com `fetchSupabase` stubado, e smoke de versão/ajuda), mais conferência visual nos temas claro e escuro. **WebKit/Safari não foi testado** — a mudança não toca em Service Worker além do bump nem em nada específico de engine, mas a lacuna existe. Deploy em 31/jul/2026 (`c84c678`), produção conferida no Chrome (rodapé v77, `buildDiariaDates` presente, série a partir de sábado começando na segunda, zero `pageerror`) e **validado pelo usuário no mesmo dia**.
...
**O que observar no retorno dela:** (a) ela tem **55 notas de ocorrência** escritas no campo antigo — o caminho de aproveitá-las é o botão 📌 (no pedido aberto) ou a sugestão da última nota (no fornecedor sem nota fixa); se ela achar trabalhoso fazer uma a uma, uma ação em massa é a segunda leva natural. (b) Os cards de agendas já tratadas continuam se acumulando no Painel (44 no tenant dela) — fora de escopo por decisão, mas é o próximo incômodo provável. (c) O modal de tratar agenda ganhou barra de ação fixa; confirmar que não atrapalhou quem já tinha o hábito de rolar.

**Retorno da compradora sobre o v77** — o v77 foi validado em produção pelo usuário em 31/jul/2026 (funcionando). Falta só o retorno de quem sugeriu a funcionalidade, sobre a rotina real dela. Dois pontos a observar quando ela usar: (a) ⚠️ **mudança de comportamento padrão** — quem usava Diária esperando os 7 dias passa a receber 5; está dito na nota de versão, mas "sumiu o sábado" não é bug, é só marcar o checkbox; (b) se o retrabalho de recriar as séries antigas incomodar, a opção (B) do spec — botão "remover fins de semana desta série" — é a segunda leva natural.

**Validação do v75/v76 (27/jul)** — confirmar com a Total Socorro que a compradora conseguiu excluir a série inteira pela UI: recarregar o portal (rodapé deve mostrar v76), abrir o compromisso recorrente, marcar **"Toda a série (17)"** e Excluir. Se o rodapé continuar em versão antiga (SW preso), usar `/?limpar=1`. Conferir depois no banco que a série `cb6286a9-5de6-4a03-b1a1-bf461dc9a6dc` (tenant `57e02e64-d14d-4049-8d80-2618df06a546`) zerou. Minors que sobraram do review de 21/jul: backfill usa `getSettings().tenantId` em vez do tenant capturado; `openGenericEventDetail` faz 2 fetches paginados onde 1 bastaria.
